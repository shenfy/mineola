...
glm::vec3 ParseVec3(const std::string &src) {
    glm::vec3 result;
    std::vector<std::string> str_vec;
    boost::algorithm::split(str_vec, src, boost::algorithm::is_any_of(","));
...
glm::vec4 ParseVec4(const std::string &src) {
    glm::vec4 result;
    std::vector<std::string> str_vec;
    boost::algorithm::split(str_vec, src, boost::algorithm::is_any_of(","));
...
glm::mat4 ParseMat4(const std::string &src) {
    glm::mat4 result;
    std::vector<std::string> str_vec;
    boost::algorithm::split(str_vec, src, boost::algorithm::is_any_of(","));
    int index = 0;
